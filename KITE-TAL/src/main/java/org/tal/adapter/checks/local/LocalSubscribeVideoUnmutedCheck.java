...
public class LocalSubscribeVideoUnmutedCheck extends TestCheck {
    ...
    String remoteId;
    public LocalSubscribeVideoUnmutedCheck(Runner runner, String remoteId) {
        super(runner);
        ...
        try {
            ...
            for (int elapsedTime = 0; elapsedTime < this.checkTimeout; elapsedTime += 5*this.checkInterval) {
                ...
                if (!"true".equalsIgnoreCase(VideoEnabled) && !"video".equalsIgnoreCase(videoCheck)) {
                    ...
                    logger.info("订阅视频enabled为：" + VideoEnabled);
            ...
            if(!VideoEnabled.equalsIgnoreCase("true" )){
                throw new KiteTestException("订阅视频enabled状态为："+VideoEnabled,Status.FAILED);
            }
        } catch (Exception e) {
